    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Configure os planos de assinatura do SGO</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {rows.map(r => (
            <div key={r.id} className={clsx(
              'bg-slate-800 rounded-xl border p-5 relative',
              r.ativo ? 'border-slate-700 hover:border-orange-500' : 'border-slate-700/50 opacity-60'
            )}>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className={clsx('text-xs px-2 py-0.5 rounded-full', r.ativo ? 'bg-emerald-900/50 text-emerald-300' : 'bg-slate-700 text-slate-400')}>
                  {r.ativo ? 'Ativo' : 'Inativo'}
                </span>
              </div>

              <h3 className="text-lg font-bold text-white">{r.nome}</h3>
              {r.descricao && <p className="text-xs text-slate-400 mt-1 mb-3">{r.descricao}</p>}

              <p className="text-2xl font-bold text-orange-400 my-3">
                {r.preco_mensal === 0 ? 'Grátis' : `R$ ${r.preco_mensal.toLocaleString('pt-BR')}`}
                {r.preco_mensal > 0 && <span className="text-sm text-slate-400 font-normal">/mês</span>}
              </p>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl border border-slate-700 w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-slate-700">
              <h2 className="text-white font-semibold">{editId ? 'Editar Plano' : 'Novo Plano'}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input value={form.nome} onChange={e => setForm(p => ({ ...p, nome: e.target.value }))}
                  placeholder="Ex: Pro"
                  className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-500 text-sm focus:outline-none focus:border-orange-500" />
              </div>
              <div>
              ...
              <p className="text-xs text-slate-500">Use -1 para ilimitado.</p>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="ativo" checked={form.ativo}
                  onChange={e => setForm(p => ({ ...p, ativo: e.target.checked }))}
                  className="rounded accent-orange-500" />
                <label htmlFor="ativo" className="text-sm text-slate-300">Plano ativo</label>
              </div>
            </div>
